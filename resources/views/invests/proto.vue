...
<b-container fluid>
  ...
  <b-table show-empty stacked="md" :bordered="true" :hover="true" :items="items" :fields="fields" :current-page="currentPage" :per-page="perPage" :filter="filter" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" @filtered="onFiltered">
    ...
    <template slot="monthly_quotes[0].close" slot-scope="row"> {{row.value}}</template>
    <!--           template das opcoes -->
...
export default {
  ...
  data() {
    return {
      fields: [{
        ...
        {
          key: 'monthly_quotes[0].close',
          label: 'Cotação',
          formatter: (value, key, item) => {
            return formatter.format(item.monthly_quotes[0].close);
          }
        },
  ...
  methods: {
    ...
    infoRow(item, index, button) {
      ...
      //this.modalInfo = item
      //this.modalInfo.brokerName = item.broker.name
      //$("#row-details").modal('show')
      //this.$root.$emit('bv::show::modal', 'modalInfo', button)
      this.item.toggleDetails;
    },
